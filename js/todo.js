...
Todo.prototype = {
	...
	createDetailDom: function(obj) {
		var detailDom = '<div class="task-detail"><form id = "detail-form">' +
			...
			'<h3>提醒时间</h3>' +
			'<input type="text" class = "warn-time" id="datetimepicker" value="' + obj.time + '" />' +
	...
	// 闹钟
	clockEvent: function() {
		this.$clockBox = $(".warn-box").css("display", "none");
		for(var i in this.task_list) {
			if(!this.task_list[i].checked) {
				this.clocking(this.task_list[i]);
			}
		}
	},

	clocking: function(obj) {
		var that = this;
		clearInterval(obj.timer);
		var music = false;
		obj.timer = setInterval(function() {
			var subTime = obj.weight - new Date().getTime();
			if(subTime <= 0 && subTime >= -60000 && !music) {
				if(!music) {
					var oMusic = document.getElementsByTagName("audio")[0];
					oMusic.play();
					that.$clockBox.css("display", "block")
						.find("strong").html(obj.content);
					that.$clockBox.find("span").off("click").on("click", function() {
						clearInterval(obj.timer);
						that.$clockBox.css("display", "none");
					});
				}
				music = true;

			} else if(subTime <= -60000) {
				clearInterval(obj.timer);
				that.$clockBox.css("display", "none");
			}
		}, 1000);
